...
import numpy as np
import torch
from torch._C import INSERT_FOLD_PREPACK_OPS, has_mkl
...
class BWM(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        self.g = nn.Conv2d(in_channels=self.in_channels, out_channels=self.out_channels, kernel_size=1, stride=1, padding=0)
    ...
    def forward(self, x, y):
        ...
        g_n = self.g(y)#.view(batch_size, self.inter_channels, -1)
        # print('g_n:', g_n.size())

        alpha = self.g(F.leaky_relu(h_m + g_n))

        w_m = torch.mul(h_m, alpha)

        w_m = self.g(w_m)#.view(batch_size, self.inter_channels, -1)
...
class EdgeConv2d(nn.Module):
    ...
    def __init__(self, in_channels, out_channels):
        ...
        # sobel_kernel = np.array([[-2, -1, 0], [-1, 1, 1], [0, 1, 2]], dtype='float32') /3    ## emboss
        ...
        # print('sobel_kernel: ', sobel_kernel.shape)
...
class Sobel_filter(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        self.lambda_min = torch.tensor(0.5).to(device)
        self.lambda_max = torch.tensor(1.5).to(device)    # before is 1.0 and 0.5  # 2.0 Average Acc: 0.8808
                                                                                    # F1: 0.6494
                                                                                    # Dice: 0.5055
                                                                                    # AJI: 0.1650
    ...
    def forward(self, input):
        ...
        x_temp = 1.0 - (x - self.lambda_min) / (self.lambda_max - self.lambda_min)
        ...
        x_final = torch.where((x > self.lambda_min) & (x < self.lambda_max), x_temp, self.con)
...
class ResUNet(nn.Module):
    def __init__(self, out_c, n_layers=34, pretrained=True, fixed_feature=False):
        ...
        self.BWM1 = BWM(in_channels=l[3], out_channels=l[3])
        self.BWM2 = BWM(in_channels=l[2], out_channels=l[2])
        self.BWM3 = BWM(in_channels=l[1], out_channels=l[1])
    ...
    def forward(self, x):
        ...
        x = self.BWM1(c4, o5)  # add for BWM
       
        x = o6 = self.u6(x, denoise_c3)
        # print("u6: ", x.size())
        out6 = self.ce6(x)
